...
class Q11 {
  ...
  def execute(sc: SparkSession): DataFrame = {
    ...
    val nation = TPCHQueryBenchmark.dfMap.get("nation").get
    val supplier = TPCHQueryBenchmark.dfMap.get("supplier").get
    val partsupp = TPCHQueryBenchmark.dfMap.get("partsupp").get
